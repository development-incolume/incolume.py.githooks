...
from incolume.py.githooks.detect_private_key import has_private_key, BLACKLIST
...
class TestCaseDetectPrivateKey:
    ...
    @pytest.mark.parametrize(
        ['entrance', 'expected'],
        [
            pytest.param('test_no_key.txt', True, marks=[]),
            pytest.param(
                'test_no_key.py', True, marks=[pytest.mark.xfail(strict=False)]
            ),
        ],
    )
    ...
    def test_with_private_key(self, entrance) -> NoReturn:
        ...
        assert has_private_key(test_file) is False
